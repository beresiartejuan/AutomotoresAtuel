...
export default function Products(props: ComponentProps<'section'>) {
    ...
    const products = getAllProducts().filter(product => usedCars == product.used && product.name.includes(nameFilter));
    ...
    return (
        <section {...props} className={styles.products_container}>
            ...
            <div>
                {products.length > 0 && products.map(product => (<ProductCard key={product.id} info={product} />))}
